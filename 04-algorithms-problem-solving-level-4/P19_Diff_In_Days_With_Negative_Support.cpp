...
int GetDiffInTwoDates (stDate Date1, stDate Date2, bool IncludeLastDay = false)
{
    ...
    if (!IsDate1BeforeDate2(Date1, Date2))
    {
        ...
        return (IncludeLastDay == false ? Days * SwapFlagValue : ++Days * SwapFlagValue);
}
...
int main ()
{
    ...
    short DiffBetweenDates = GetDiffInTwoDates(Date1, Date2);

        cout << "\nDifference is : " << DiffBetweenDates << " Day(s)" << endl;
        cout << "Difference (Including End Day) is : " << GetDiffInTwoDates(Date1, Date2, true) << " Day(s)";
    return 0;
}