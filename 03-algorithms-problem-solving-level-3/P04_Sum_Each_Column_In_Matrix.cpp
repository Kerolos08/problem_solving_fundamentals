...
int SumOfcolumns (int arr[3][3], int ColumnNumber, int RowNumber)
{
    int sum = 0;
...
void PrintTheSumOfEachRow (int arr[3][3], int columns, int rows)
{
    cout << "\nThe following are the sum of each row in the matrix :\n";
    for (int i = 0; i < rows; i++)
    {
        cout << "Row " << i + 1 << " Sum = " << SumOfcolumns(arr, i, rows) << endl;
    }
}
...
//     {
//     OneDimArr[i] = SumOfcolumns(TwoDimArr, i, rows);
//     }
// }
...
int main ()
{
    ...
    PrintTheSumOfEachRow(TwoDimArr,3,3);
    // int OneDimArr[3];
